...
def extract_premium(text: str) -> float:
    ...
    for label in ["total premie", "pris per år", "pris totalt", "totalpris", "totalbelopp", "premie"]:
        match = re.search(rf"(?i){label}[^0-9]{{0,40}}([0-9\s.,]+)\s*kr", text)
        if match:
            return normalize_number(match.group(1))
    return 0.0

def extract_deductible(text: str) -> str:
    match = re.search(r"(?i)(självrisk|självrisken)[^0-9]{0,40}([0-9\s.,%]+|[0-9]+\s*basbelopp)", text)
...
def extract_egendom(text: str) -> Dict:
    ...
    for typ in typer:
        match = re.search(rf"(?i){typ}[^0-9]{{0,40}}([0-9\s.,]+)\s*kr", text)
        if match:
...
def extract_ansvar(text: str) -> Dict:
    ansvar = {}
    typer = ["produktansvar", "verksamhetsansvar", "ansvarsförsäkring", "allmänt ansvar", "företagsansvar", "ansvar"]
    for typ in typer:
        match = re.search(rf"(?i){typ}[^0-9]{{0,40}}([0-9\s.,]+)\s*kr", text)
        if match:
...
def extract_karen_ansvarstid(text: str) -> Dict:
    karens = re.search(r"(?i)(karens|karensdagar)[^0-9]{0,40}(\d+\s*(dygn|dagar|timmar|tim|h))", text)
    ansvarstid = re.search(r"(?i)(ansvarstid)[^0-9]{0,40}(\d+\s*(månader|mån|dagar))", text)
    return {
